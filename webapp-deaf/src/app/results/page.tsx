...
function Histogram({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm flex flex-col gap-1">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full mt-1" aria-label={title}>
        <g transform={`translate(${ml},${mt})`}>
          ...
          {bins.map((b, i) => {
            ...
            return (
              <g key={i}>
                ...
                />
                {b.count > 0 && (
                  <text x={i * barW + barW / 2} y={ch - barH - 3} fontSize={9} textAnchor="middle" fill="#374151">
                    {b.count}
                  </text>
                )}
              </g>
          ...
          {bins.map((b, i) => (
            <text key={i} x={i * barW + barW / 2} y={ch + 13} fontSize={8} textAnchor="middle" fill="#9ca3af">
              {b.label}%
            </text>
          ))}

          <text x={cw / 2} y={ch + 32} fontSize={9} textAnchor="middle" fill="#6b7280">
            {xAxisLabel} (%)
          </text>

          <text x={-(ch / 2)} y={-18} fontSize={9} textAnchor="middle" fill="#6b7280" transform="rotate(-90)">
            # samples
          </text>
          ...
          <line
            x1={meanX} y1={0} x2={meanX} y2={ch}
            stroke={meanColor}
            strokeDasharray="4 3"
            strokeWidth={1.5}
          />
          <text x={Math.min(meanX + 4, cw - 36)} y={10} fontSize={8.5} fill={meanColor}>
...
const MODE_COLORS: Record<string, string> = {
  FILL: "bg-blue-100 text-blue-800 border-blue-200",
  RECONSTRUCT: "bg-orange-100 text-orange-800 border-orange-200",
  FILL_REVERTED: "bg-gray-100 text-gray-600 border-gray-200",
  SKIP: "bg-green-100 text-green-800 border-green-200",
};

function ModeBreakdown({ samples }: { samples: Sample[] }) {
  const modes = Object.keys(samplesData.meta.mode_legend) as Mode[];
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {modes.map((mode) => {
        const count = samples.filter((s) => s.mode === mode).length;
        const pct = Math.round((count / samples.length) * 100);
        const color = MODE_COLORS[mode] ?? MODE_COLORS.FILL_REVERTED;
        const legend = samplesData.meta.mode_legend[mode as keyof typeof samplesData.meta.mode_legend];
        return (
          <div key={mode} className={`rounded-xl border p-3 shadow-sm ${color.replace("text-", "border-").split(" ").pop()} bg-white`}>
            ...
            <p className="text-xs text-gray-400">{pct}% of samples</p>
            <p className="text-xs text-gray-500 mt-1 leading-snug">{legend}</p>
          </div>
        );
      })}
    </div>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <header className="text-center space-y-1">
          ...
          <p className="text-xs text-gray-400">
            {n} test samples — Deaf speech (Marathi)
          </p>
        </header>
        ...
        <section>
          ...
          <div className="flex gap-3">
            ...
            <StatPill
              ...
              value={`${meanWerAfter}%`}
              sub={
                meanWerAfter < meanWerBefore
                  ? `↓ ${meanWerBefore - meanWerAfter}pp improvement`
                  : "no change"
              }
        ...
        <section>
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">
            Post-processing mode breakdown
          </p>
          <ModeBreakdown samples={samples} />
        </section>

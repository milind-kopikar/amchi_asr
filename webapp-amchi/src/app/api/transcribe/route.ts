 ...
 * RunPod serverless endpoint for the Amchi Konkani ASR (Run S model, 34.1% WER).
 *
 ...
 *   RUNPOD_AMCHI_ENDPOINT_ID      (required)
 *
...
export async function POST(req: NextRequest) {
  ...
  // 2. Forward to RunPod via the thin client
  const result = await callRunpodTranscribe(
    process.env.RUNPOD_API_KEY ?? "",
    process.env.RUNPOD_AMCHI_ENDPOINT_ID ?? "",
    body.audio_base64,
  );

...
class _LanguageMaskList(list):
    """Wrapper for language_masks to support tensor indices robustly."""
    def __init__(self, masks):
        super().__init__(list(masks))

    def __getitem__(self, idx):
        if isinstance(idx, torch.Tensor):
            # support scalar tensors and 1-D tensors of single element
            if idx.numel() == 1:
                idx = int(idx.item())
            else:
                idx = int(idx.long().view(-1)[0].item())
...
class ConvASRDecoder(NeuralModule, Exportable, adapter_mixins.AdapterModuleMixin):
    ...
    def __init__(self, feat_in, num_classes, init_mode="xavier_uniform", vocabulary=None, multisoftmax=False, language_masks=None): #CTEMO
        ...
        if language_masks is None:
            ...
            try:
                self.language_masks = _LanguageMaskList(language_masks)
            except Exception:
    ...
    def forward(self, encoder_output, language_ids=None): #CTEMO
        ...
        if language_ids is not None:
            # Normalize language_ids to a sequence matching the batch size
            B, T, C = decoder_output.shape
            try:
                if not hasattr(language_ids, "__len__"):
                    language_ids = [language_ids] * B
                else:
                    if len(language_ids) == 1 and B > 1:
                        language_ids = [language_ids[0]] * B
            except Exception:
                language_ids = [language_ids] * B

            masks = []
            for lid in language_ids:
                # Normalize language id to Python int (handle torch scalar tensors)
                try:
                    if isinstance(lid, torch.Tensor):
                        if lid.numel() == 1:
                            lid = int(lid.item())
                        else:
                            lid = int(lid.long().view(-1)[0].item())
                    else:
                        lid = int(lid)
                except Exception:
                    lid = int(lid)

                # Retrieve language mask entry; default to select-all if unavailable
                try:
                    lm = self.language_masks[lid]
                except Exception:
                    lm = torch.arange(C, dtype=torch.long)

                # Convert language mask to 1-D boolean mask of length C
                if isinstance(lm, torch.Tensor):
                    if lm.dtype == torch.bool and lm.dim() == 1 and lm.numel() == C:
                        mask1 = lm.to(dtype=torch.bool, device=decoder_output.device)
                    else:
                        idxs = lm.long().to(device=decoder_output.device)
                        idxs = idxs[(idxs >= 0) & (idxs < C)]
                        m = torch.zeros(C, dtype=torch.bool, device=decoder_output.device)
                        if idxs.numel() > 0:
                            m[idxs] = True
                        mask1 = m
                else:
                    try:
                        arr = torch.tensor(lm, dtype=torch.bool, device=decoder_output.device)
                        if arr.dim() == 1 and arr.numel() == C:
                            mask1 = arr
                        else:
                            idxs = torch.tensor(lm, dtype=torch.long, device=decoder_output.device)
                            idxs = idxs[(idxs >= 0) & (idxs < C)]
                            m = torch.zeros(C, dtype=torch.bool, device=decoder_output.device)
                            if idxs.numel() > 0:
                                m[idxs] = True
            ...
            # stack masks -> [B, C], then expand to [B, T, C]
            sample_mask = torch.stack(masks, dim=0)
            sample_mask = sample_mask.unsqueeze(1).repeat(1, T, 1).to(decoder_output.device)
            # Apply mask and return masked selection
            decoder_output = torch.masked_select(decoder_output, sample_mask).view(B, T, -1)
        # print(mask[0][0])
